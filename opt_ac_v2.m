...
if isempty(acp_v) == 0
    for i=1:size(acp_v,2)
        Constraints=[Constraints
            acp_strg(1,i) <= 0.4*acp_adopt(i)
            acp_strg(2:length(acp_strg),i) == acp_v(7,i).*acp_strg(1:length(acp_strg)-1,i) + acp_chrg(:,i) %%% Energy Balance for the AC
            acp_strg(:,i) <= acp_v(6,i)*acp_adopt(i)]; %%%AC Storage is limited by adopted chiller size

        
        if acp_v(end) == 1
            Constraints=[Constraints
                acp_cool(1:winter_time_count,i) <= (1/acp_v(6,i)).*acp_strg(1:winter_time_count,i)
                acp_cool(winter_time_count+1:endpts(length(endpts)),i) <= acp_adopt(i)
                acp_cool(1:winter_time_count,i) <= (2*cooling_max)*acp_op(:,i)
                acp_v(8,i)*acp_adopt(i) <= acp_strg(:,i) + 2*cooling_max*(1-acp_op)];
                
           
        elseif acp_v(end) == 0
            Constraints=[Constraints
                acp_cool(:,i) <= (1/acp_v(6,i)).*acp_strg(:,i)
                acp_cool(:,i) <= (2*cooling_max)*acp_op(:,i)
                acp_cool(:,i) <= (2*cooling_max)*acp_op(:,i)
                acp_v(8,i)*acp_adopt(i) <= acp_strg(:,i) + 2*cooling_max*(1-acp_op)];
