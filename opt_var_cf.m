...
if isempty(boil_v)==0
    ...
    Objective=Objective+ boil_v(1)*sum(boil);
...
%     index2_vc=[max(max(index2_boil))+1 max(max(index2_boil))+endpts(length(endpts))];
else
...
if isempty(acp_v) == 0
    ...
    if acp_v(end) == 1
        ...
        %%% ACp discharging
        acp_dchrg=sdpvar(winter_time_count-1,size(acp_v,2),'full');
        %%%ACp Storage
        acp_strg=sdpvar(winter_time_count,size(acp_v,2),'full');
        %%%ACp Operaiton
        acp_dchrg_op=binvar(winter_time_count-1,size(acp_v,2),'full');
        %%%ACp Operaiton
        acp_chrg_op=binvar(winter_time_count-1,size(acp_v,2),'full');
        
%         %%%ACp Operaiton
%         acp_op=binvar(winter_time_count-1,size(acp_v,2),'full');
    elseif acp_v(end) == 0
        %%% ACp charging
        acp_chrg=sdpvar(endpts(length(endpts))-1,size(acp_v,2),'full');
        %%% ACp discharging
        acp_dchrg=sdpvar(endpts(length(endpts))-1,size(acp_v,2),'full');
        %%%ACp Storage
        acp_strg=sdpvar(endpts(length(endpts)),size(acp_v,2),'full');
        %%%ACp Operaiton
        acp_dchrg_op=binvar(endpts(length(endpts))-1,size(acp_v,2),'full');
        %%%ACp Operaiton
        acp_chrg_op=binvar(endpts(length(endpts))-1,size(acp_v,2),'full');
    ...
    acp_dchrg=zeros(endpts(length(endpts)),1);
end